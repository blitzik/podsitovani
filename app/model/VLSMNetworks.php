...
namespace Model;
	...
	class VLSMNetworks
	{
		...
		{
			$networkAddress = $this->network->getNetworkAddress();
			$subnetMask = new SubnetMask($this->calcCIDRbasedOnNumberOfHosts($this->networkHosts[0]));
			$this->subnetworks[0] = new Network($networkAddress, $subnetMask);

			for ($i = 0; $i < count($this->networkHosts); $i++) {

				if (($i + 1) > (count($this->networkHosts) - 1)) {break;}
				$newCIDR = $this->calcCIDRbasedOnNumberOfHosts($this->networkHosts[$i + 1]);
				$newMask = new SubnetMask($newCIDR);
				$this->subnetworks[$i + 1] = new Network($this->findNextNetworkAddress($this->subnetworks[$i]->getBroadcastAddress()), $newMask);
			}
		}
		...
		public function isRangeOfAddressesBigEnough()
		{
